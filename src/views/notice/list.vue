  ...
  <div class="fileList">
    ...
    >
      <el-table-column
        ...
        align="center"
      />
      <el-table-column prop="senderId" label="发布者" />
      <el-table-column prop="title" label="标题" />
      <el-table-column prop="title" label="是否置顶" />
      <el-table-column prop="sendTime" label="时间" sortable>
        <!-- <template #default="scope">{{scope.row.time.split(".")[0].split("T").join(" ")}}</template> -->
      </el-table-column>
      <el-table-column prop="attachment" label="操作" min-width="100">
        <template #default="scope">
          ...
          >
            <div>
              <el-button auto-insert-space @click="check(scope.$index)"
                >查看</el-button
              >
              <el-button auto-insert-space @click="edit(scope.$index)"
                >回复</el-button
              >
              <el-popconfirm
                title="您确定要删除这个栏目吗？"
    ...
    <div v-if="isChecking" class="check">
      <div class="write_title">
        ...
        <div class="check_title_content">
          {{ tableData[this.current].senderId }}
        </div>
      </div>
      <div class="write_title">
  ...
  methods: {
    ...
    checkReply(isReply) {
      ...
      for (let i = 0; i < this.data.length; i++) {
        if (this.checkIsReply) {
          //筛选出已回复的站内信 status==0已回复
