<template>
  <div id="swiper" class="swiper">
    <div class="box">
      ...
      <transition-group name="ani5">
        <div
          :style="{
            transition: transOX
              ? `height .18s linear, transform ${transdur}s ease`
              : 'height .18s linear',
          }"
          :ref="`item` + i"
        ...
        >
          ...
          <div>
            <img
              class="thumb"
              :style="{ opacity: i == cur ? 1 : 0.8 }"
              :src="ENV_d ? 'http://101.42.225.75:1234'+o.picturePath:o.picturePath"
              alt=""
            />
          </div>
          <div>
            <div class="btn" @click="ontop(i)">
              置顶
              <VividClick />
            </div>
    ...
    <teleport to="body">
      <Popup @submit="submit" @destroy="popupOX=false" v-if="popupOX" />
      <Popup_ :carouselId="popup_props.carouselId" :fileId_from="popup_props.fileId_from" :href_from="popup_props.href_from" :title_from="popup_props.title_from" :ImgSrc_from="popup_props.ImgSrc_from" @submit="submit" @destroy="popupOX_=false" v-if="popupOX_" />
    </teleport>
  </div>
</template>
...
import Popup_ from "@/components/ljc/swiper/Popup_.vue";
import request from "@/utils/pub-use.ts";
import {getCarousel,topCarousel,deleteCarousel} from '@/api/swiper.ts';
export default {
  components: {
    VividClick,
    FullView,
  ...
  data() {
    return {
      ...
      ENV_d: (process.env.NODE_ENV === 'development'),
    };
  },
  methods: {
    edit(index){
      this.popup_props.carouselId = this.assets[index].carouselId
      this.popup_props.href_from = this.assets[index].link
      this.popup_props.title_from = this.assets[index].title
      this.popup_props.ImgSrc_from = this.assets[index].picturePath
      this.popup_props.fileId_from = this.assets[index].fileId
    ...
    deletee(index){
      deleteCarousel(this.assets[index].carouselId).then(v=>{
    ...
    ontop(index) {
      topCarousel(this.assets[index].carouselId).then(v=>{
        console.log(v)
