<template>
  <div id="swiper" class="swiper">
    ...
    <teleport to="body">
      <Popup_notice @submit="submit" @destroy="popupOX = false" v-if="popupOX" />
      <Popup_notice_ @submit="submit" :libraryFileId="popup_props.libraryFileId" :title_="popup_props.title" :text="popup_props.text" @destroy="popupOX_ = false" v-if="popupOX_" />
    </teleport>
  </div>
</template>
...
import Popup_notice from "@/components/ljc/notice/Popup_notice.vue";
import Popup_notice_ from "@/components/ljc/notice/Popup_notice_.vue";
import request from "@/utils/pub-use.ts";

import { getNotices, topNoticeItem, deleteNotice } from "@/api/notice.ts";
export default {
  components: {
    VividClick,
    FullView,
  ...
  methods: {
    edit(index){
      this.popup_props.libraryFileId = this.assets[index].libraryFileId
      this.popup_props.title = this.assets[index].title
      this.popup_props.text = this.assets[index].text
    ...
    deletee(index){
      deleteNotice(this.assets[index].libraryFileId).then(v=>{
        console.log(v)
    ...
    ontop(index) {
      // console.log(this.assets[index].libraryFileId)
      topNoticeItem(this.assets[index].libraryFileId).then(v=>{
        console.log(v)
