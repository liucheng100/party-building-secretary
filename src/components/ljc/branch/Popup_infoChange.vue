    ...
    <div class="popup" @click="destroy($event)" :style="{backgroundColor: `rgba(0,0,0,${aniL.alph})`}">
      <div class="boat" :style="{transform: `translateY(${aniL.y})`, opacity: aniL.alph1}" @click.stop>
        <div class="box">
          ...
          <div v-if="multiSno" class="item-select warning">
            将对以下选中目标进行统一操作:
          </div>
          <div v-if="multiSno" class="item-select wrapflex">
            <div v-for="i in number" class="snoItem">
              {{ i.name }} {{ i.sno }}
            </div>
          </div>
          <div v-if="!multiSno" class="item-select">
            <span>学号</span>
            <div class="elinput">
              <el-input @change="catchNum" v-model="selectValue.v_sno" size="large" placeholder="" />
            </div>
          </div>
          <div v-if="!multiSno" class="item-select">
            <span>姓名</span>
            <div class="elinput">
              <el-input disabled v-model="selectValue.n_name" size="large" placeholder="根据学号自动生成" />
            </div>
          </div>
          <div v-if="!multiSno" class="item-select">
            <span>当前学院</span>
            <div class="elinput">
              <el-input disabled v-model="n_depart" size="large" placeholder="根据学号自动生成" />
            </div>
          </div>
          <div v-if="!multiSno" class="item-select">
            <span>
            ...
            <div class="elinput">
              <el-input disabled v-model="n_branch" size="large" placeholder="根据学号自动生成" />
            </div>
          </div>
          <div class="item-select">
            ...
            <el-select @change="getBranch" v-model="selectValue.v_department" class="m-2 elinput" placeholder="Select" size="large">
              <el-option
          ...
          <div class="item-select">
            ...
            <el-select v-model="selectValue.v_branch" class="m-2 elinput" placeholder="Select" size="large">
              <el-option
                v-for="item in selectOption.op_branch"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
  ...
  methods:{
    ...
    submit(){
      ...
      pushArray({
        ...
        targetBranchId: this.selectValue.v_branch
      }).then(v=>{
        console.log(v)
        if(!v.code){
