...
import { alterRecommend } from "@/api/recommendList.ts";
export default{
  name: 'Popup',
  props:{
    recommendedFileId:{
  ...
  methods:{
    ...
    submit(){
      alterRecommend({
        recommendedFileId: this.recommendedFileId,
        newFileId: this.chosenFile.libraryFileId
      }).then(v=>{
        console.log(v)
        this.$emit('submit')
        this.destroy_1()
